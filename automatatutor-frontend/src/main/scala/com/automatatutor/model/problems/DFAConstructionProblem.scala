...
class DFAConstructionProblem extends LongKeyedMapper[DFAConstructionProblem] with IdPK with SpecificProblem[DFAConstructionProblem] {
  ...
  override def toXML(): Node = {
    return <DFAConstructionProblem>
             <Automaton>{ this.getAutomaton }</Automaton>
           </DFAConstructionProblem>
  }
}

object DFAConstructionProblem extends DFAConstructionProblem with SpecificProblemSingleton with LongKeyedMetaMapper[DFAConstructionProblem] {
