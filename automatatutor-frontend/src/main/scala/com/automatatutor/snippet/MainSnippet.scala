...
class Mainsnippet {
  ...
  def personalsection(ignored: NodeSeq): NodeSeq = {
	...
	if (user.isAdmin) return autogenButton ++ usersButton
	if (user.isInstructor) return autogenButton ++ editProfilButton
	return return autogenButton ++ editProfilButton ++ becomeInstructorButton
  }
