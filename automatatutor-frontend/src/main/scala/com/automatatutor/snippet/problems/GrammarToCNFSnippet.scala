...
object GrammarToCNFSnippet extends SpecificProblemSnippet {
  ...
  override def renderCreate(createUnspecificProb: (String, String) => Problem,
    ...
    val template: NodeSeq = Templates(List("templates-hidden", "grammar-to-cnf-problem", "create")) openOr Text("Could not find template /templates-hidden/grammar-to-cnf-problem/create")
  ...
  override def renderSolve(generalProblem: Problem, maxGrade: Long, lastAttempt: Box[SolutionAttempt],
    ...
    def grade(attemptGrammar: String): JsCmd = {
      ...
      val attemptGrammarReplaced = attemptGrammar.replaceAll("(?i)(\\\\epsilon|\\\\eps|\\\\e)", "_")
      ...
      val gradeAndFeedback = GraderConnection.getGrammarToCNFFeedback(specificProblem.grammar.is, attemptGrammarReplaced, maxGrade.toInt)
	  ...
	  if (!validAttempt) { 
		...
		if (generalAttempt != null) {
          GrammarToCNFSolutionAttempt.create.solutionAttemptId(generalAttempt).attemptGrammar(attemptGrammarReplaced).save
        }
	  }

      val setNumericalGrade: JsCmd = SetHtml("grade", Text(numericalGrade.toString + "/" + maxGrade.toString))
      val setFeedback: JsCmd = SetHtml("feedback",
        Grammar.preprocessFeedback(gradeAndFeedback._2))
    ...
    val template: NodeSeq = Templates(List("templates-hidden", "grammar-to-cnf-problem", "solve")) openOr Text("Could not find template /templates-hidden/grammar-to-cnf-problem/solve")
