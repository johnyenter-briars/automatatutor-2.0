...
class Usersnippet extends SortedPaginatorSnippet[User, BaseOwnedMappedField[User]] {
  ...
  val searchParamsForPage: Seq[QueryParam[User]] = sortQueryPara :: (MaxRows[User](itemsPerPage) :: (StartAt[User](curPage * itemsPerPage) :: searchParams))
  ...
  def makeinstructor(xhtml: NodeSeq): NodeSeq = {
    ...
    val body = <div>
      ...
      <p>your account (
        {user.email}
        ) is now an instructor. Go ahead and
