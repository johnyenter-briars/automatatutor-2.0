...
$.SvgCanvas = function(container, config, style) {
	...
	//manage svg elements according to device
	var effectiveWidth = window.screen.width;
	//controls svgs for nodes 
	var node_radius,
		loop_Width,
		loop_Height,
		//reflexive label for DFA
		refl_label_fontsize,
		refl_label_buffer,
		//controls hovermenu for NFA
		hovermenu_radius,
		hovermenu_step,
		hovermenu_buffer_all,
		hovermenu_buffer = {x:0, y:0},
		//controls clearRect and clearText
		clearRect_text,
		clearRect_text_fontsize,
		clearRect_text_anchor,
		clearRect_text_pos = {x:0, y:0},
		clearRect_width,
		clearRect_height,
		//controls for canvas width
		canvas_width_buffer,
		// controls for hiddentrash
		hiddentrash_radius;
	if(effectiveWidth < 500) //Smartphone
	{
		node_radius = 25;
		loop_Width = Math.PI / 2;
		loop_Height = 100;
		refl_label_fontsize = 70;
		refl_label_buffer = 50;
		hovermenu_radius = 70;
		hovermenu_step = Math.PI/3;
		hovermenu_buffer_all = 10;
		hovermenu_buffer.x = 35;
		hovermenu_buffer.y = 30;
		clearRect_width = 80;
		clearRect_height = 80;
		clearRect_text = '\u21BB'; //reload symbol
		clearRect_text_anchor = "middle";
		clearRect_text_fontsize = 60;
		clearRect_text_pos.x = clearRect_width/2 - 2;
		clearRect_text_pos.y = clearRect_text_fontsize;
		canvas_width_buffer = 0.88;
		hiddentrash_radius = 100;
	}
	else if(effectiveWidth < 900) //Tablets
	{
		node_radius = effectiveWidth/25; //35
		loop_Width = Math.PI / 2;
		loop_Height = 90;
		refl_label_fontsize = 50;
		refl_label_buffer = 40;
		hovermenu_radius = 70;
		hovermenu_step = Math.PI/3;
		hovermenu_buffer_all = 10;
		hovermenu_buffer.x = 35;
		hovermenu_buffer.y = 30;
		clearRect_text = 'Reset Canvas';
		clearRect_text_anchor = "middle";
		clearRect_text_fontsize = 16;
		clearRect_width = 115;
		clearRect_height = 36;
		clearRect_text_pos.x = clearRect_width/2 - 2;
		clearRect_text_pos.y = clearRect_text_fontsize + 5;
		canvas_width_buffer = 0.88;
		hiddentrash_radius = 100;
	}else //Desktop
	{
		...
		loop_Width = Math.PI / 2;
		loop_Height = 80;
		refl_label_buffer = 15;
		refl_label_fontsize = 16;
		hovermenu_radius = 20;
		hovermenu_step = Math.PI/6;
		hovermenu_buffer_all = 0;
		hovermenu_buffer.x = 10;
		hovermenu_buffer.y = 10;
		clearRect_text = 'Reset Canvas';
		clearRect_text_anchor = "left";
		clearRect_text_pos.x = 98;
		clearRect_text_pos.y = 18;
		clearRect_text_fontsize = 15;
		clearRect_width = 105;
		clearRect_height = 26;
		canvas_width_buffer = 1;
		hiddentrash_radius = 100;
	}

	var globalConfig = {
		node: {
			radius: node_radius/*15*/,		// Used when rendering the node as a circle
		...
		transition: {
			loopWidth: loop_Width/*Math.PI / 2*/,	// The distance between the two points at which a loop connects to its vertex (in radians)
			loopHeight: loop_Height/*80*/ // The height of a looping transition
		}
	}
	...
	function populateHoverMenusWithAlphabet(menus) 
	{
		...
		function calculateXCoordinate(symbolIndex) {
			...
			return polarToPlanar(config.node.radius + hovermenu_buffer.x, angle).x;
		}
		...
		function calculateYCoordinate(symbolIndex) {
			...
			return polarToPlanar(config.node.radius + hovermenu_buffer.y, angle).y + 5 + hovermenu_buffer_all;
	    }
		...
		for(var i = 0; i < alphabet.length; i++){
			menus.append('svg:text')
			    ...
			    .on('mousedown touchstart', onLabelMousedown)
    ...
    //Set up width/height to be dynamic for mobile devices
    var dynamic_width,
    	dynamic_height;
    if(window.screen.width < 500) //Smartphone
	{
		dynamic_width = canvas_width_buffer * window.screen.width;
		dynamic_height = 0.88 * window.screen.height;
	}
	else if(window.screen.width < 900) //Tablets
	{
		dynamic_width = canvas_width_buffer*window.screen.width;
		dynamic_height = 0.88 * window.screen.height;
	}else //Desktop
	{
		dynamic_width = config.dimensions[0];
		dynamic_height = config.dimensions[1];
	}
    // set up SVG for D3
    var width = dynamic_width;
    var height = dynamic_height;
	...
	//prevent zooming while touch dragging
	function nozoom() 
	{
    ...
    //d3 changes
    var simulation;
    var layoutForce = "spring"; //testing var for forces: eades, frurein, d3
    //variables used for radial force
    var radius = (Math.sqrt(Math.pow(width, 2) + Math.pow(height, 2)))/4 
					- config.transition.loopHeight,
	...
	.attr('class', 'delete')
	.attr('width', clearRect_width) //105
	.attr('height', clearRect_height) //26
	.attr('x', width-(clearRect_width-1)) //width-104
	.attr('y', -1)
	.on('mouseover touchstart', function() {
	    overClear = true;
	    clearRect.attr('width', (clearRect_width + 5)) //110
		.attr('height', (clearRect_height + 3)) //29
		.attr('x', width - (clearRect_width + 5)); //109

	    clearText.attr('x', width - (clearRect_text_pos.x+2))//100
		.attr('y', (clearRect_text_pos.y+2)); //20
	})
	.on('mouseout touchend', function() {
	    overClear = false;

	    clearRect.attr('width', clearRect_width) //105
		.attr('height', clearRect_height) //26
		.attr('x', width - (clearRect_width-1)); //104

	    clearText.attr('x', width - clearRect_text_pos.x) //98
		.attr('y', clearRect_text_pos.y); //18
	})
	...
	.text(clearRect_text)
	.style("font-size", clearRect_text_fontsize+"px")
	.style("text-anchor", clearRect_text_anchor)
	.attr('x', width - clearRect_text_pos.x) //98
	.attr('y', clearRect_text_pos.y) //18 
	...
	labels.attr('x', function(d) {
	    if(d.reflexive) {
			var angle = 0;
        	if(layoutForce == "radial" && d.source.useRadial)
	    	{
	    		for (var i = 0; i < radial_angles.length; i++) 
	    		{
	    			if(radial_angles[i].id == d.source.id) 
	    			{
	    				angle = Math.PI - radial_angles[i].alpha;
			    		if (d.source.flip)
			        	{
			            	angle = 2 * Math.PI - radial_angles[i].alpha;
	    	...
	    	{
	    		angle = d.source.selfLoopangle + 1 * Math.PI;//Math.PI / 2;
           		if (d.source.flip)
           		{
                	angle = d.source.selfLoopangle;//3 * Math.PI / 2;
            	}
	    	}
	    	
            if (layoutForce == "radial" && d.source.useRadial) 
            {
            	var x = Math.round(d.source.x + 25*Math.cos(angle) + config.node.radius * Math.cos(angle));
            	var y = Math.round(d.source.y + 25*Math.cos(angle) + config.node.radius * Math.sin(angle));
            } 
            else
            {
            	var x = Math.round(d.source.x + 15*Math.cos(angle) + config.node.radius * Math.cos(angle));
            	var y = Math.round(d.source.y + 15*Math.cos(angle) + config.node.radius * Math.sin(angle));
            }
			...
			{
				//make dynamic for mobile
			    return x + (ax + bx) / 2 + ((d.linknum - (.5 + d.source.reflexiveNum/2 ))/d.source.reflexiveNum)*(refl_label_buffer/*15*/*d.source.reflexiveNum);
			}
			else 
			{
			    return x + (ax + bx) / 2;
    ...
    .attr('y', function(d) {
		if(d.reflexive) {
            ...
            if (layoutForce == "radial" && d.source.useRadial) 
            {
            	var x = Math.round(d.source.x + 20*Math.sin(angle) + config.node.radius * Math.cos(angle));
            	var y = Math.round(d.source.y + 20*Math.sin(angle) + config.node.radius * Math.sin(angle));
            } 
            else
            {
            	var x = Math.round(d.source.x + 15*Math.sin(angle) + config.node.radius * Math.cos(angle));
            	var y = Math.round(d.source.y + 15*Math.sin(angle) + config.node.radius * Math.sin(angle));
            }
    ...
    function restart()
    {
    	//used for dynamic drawing of refl Links
    	setselfLoopangles(links);
		...
		var h = hoverPath.enter().append('svg:path')
		    ...
		    .on('mousedown touchstart', mousedownPath);
		hoverPath.exit().remove();
		//d3 changes
		hoverPath = hoverPath.merge(h);

		// path (link) group
		path = path.data(links, d => '' + d.source + d.target);
		...
		// add path labels
		labels = labels.data(links, d => '' + d.source + d.target);

		labels.classed('hidden', function(d) { return d === hidden_link; })
		    .text(function(d) { return makeLabel(d.trans); });

		//l denotes the new elements, which we will merge with the existing ones down below
		var l = labels.enter().append('svg:text')
		    .attr('class', 'transLabel')
		    .text(function(d) { return makeLabel(d.trans); })
		    .style("font-size", refl_label_fontsize + "px")
		    .on('mouseover', function(d, i) {
			hover_link = d;
			menu_link = hover_link;
			hover_label = true;
		    ...
		    .on('mousedown touchstart', function(d, i)
		    {
		    	// Ignore if rightclick
		...
		//small control variable for touch
		var no_mouseover = false; //could be set true in mousemove for example for uses with touch and mouse combined
		...
		{
			//simulate mouseover for touch input
			if(d3.event.type == "touchstart")
			{
				...
				no_mouseover = true;
				menu_node = d;
				//make hovermenu toggleable for touch input
				d.menu_visible = !d.menu_visible;
				showMenu = !showMenu;
				d3.select(this).attr('transform', 'scale(1.1)');
				d3.select(this.parentNode).selectAll('#marker').attr('transform', 'scale(1.1)')
			}
		...
		g.append('svg:circle')
		    ...
		    .on('touchstart', onNodeMousedown)
		    .on('touchmove', onNodeMousedown)
		    .on('touchend', onNodeMouseup);
		...
		if(useHoverMenu() && !solveMode) {
		    ...
		    menus.append('svg:circle')
				.attr('class', 'hoverMenu visible')
				.classed('visible', isHoverMenuVisible)
				.attr('r', config.node.radius + hovermenu_radius)
				.on('mouseover', function(d) 
				{
				    showMenu = true;
				})
		    ...
		    //d3 changes
	    	hoverMenu = hoverMenu.merge(menus);
		}
...
/*
    * Container function for all behaviors regarding d3 forces
    */
    function applyForces()
    {
		...
		//custom force: bind node within canvas
		function canvas_force() { 
		  for (var i = 0, n = nodes.length; i < n; ++i) {
		    curr_node = nodes[i];
		    curr_node.x = Math.max(config.node.radius, Math.min(width - config.node.radius, curr_node.x));
		    curr_node.y = Math.max(config.node.radius, Math.min(height - config.node.radius, curr_node.y));

		  }
		}

		//switches between which layout force to use
		switch(layoutForce) 
		{
			case "spring":
    			function spring(alpha)
				{
					return graphLayouting.springembedder(alpha, nodes, links, width, height);
				}
				simulation.force("spring_force", spring);
	        	break;
    		case "eades":
    			function eades(alpha)
				{
					return graphLayouting.eades(alpha, nodes, links);
				}
				simulation.force("eades_force", eades);
	        	break;
    		case "frurein":
	        	function frurein(alpha)
				{
					return graphLayouting.frurein(alpha, nodes, links, width, height);
				}
				simulation.force("frurein_force", frurein);
	        	break;
	        case "radial":
    			function radial(alpha)
				{
					radius = (Math.sqrt(Math.pow(width, 2) + Math.pow(height, 2)))/4 
					- config.transition.loopHeight;
					center = {x: width/2, y: height/2};
					var radialNodes = nodes.filter(function isRadial(node, i, nodes)
						{
							if(node.useRadial)
								return node;
						});
					var obj = graphLayouting.radial(alpha, radialNodes, links, center, radius);
					radial_angles = graphLayouting.radial_angles();
					return obj;

				}
				
				simulation.force("radial_force", radial);
	        	break;
        	case "frureinvar":
			function frureinvar(alpha)
			{
				var obj = graphLayouting.frureinvar(alpha, nodes, links);
				return obj;

			}
			
			simulation.force("frureinvar_force", frureinvar);
			simulation.force("center_force", d3.forceCenter(width / 2, height / 2));
        	break;
        	//force algo in d3.forceManyBody/d3.forceLink
    		default:
    			/*
				* adding forces
				* charge_force for repelling force between nodes
				* center_force for gravitational force towards center
				*/
    			simulation
		    		.force("charge_force", d3.forceManyBody());
		
	        	//create new links array for better use in d3
				var link_data = [];
			    for (var i = 0; i < links.length; i++)
			    {
			    	if(links[i].source.id != links[i].target.id)
			    	{
			    		link_data.push({source: links[i].source, target: links[i].target});
			    	}
		...
		simulation.force("canvas_force", canvas_force);
    ...
    function drawSelfLoop(transition) {
		//radial force changes orientation of self loops
		if(layoutForce == "radial" && transition.source.useRadial)
		{
			for (var i = 0; i < radial_angles.length; i++) 
			{
				if(radial_angles[i].id == transition.source.id) 
				{
					angle = Math.PI - radial_angles[i].alpha;
		...
		else
		{
			...
			angle = transition.source.selfLoopangle + 1 * Math.PI;//Math.PI / 2;
	    	if (transition.source.flip)
	    	{
	        	angle = transition.source.selfLoopangle;//3 * Math.PI / 2;
	    	}
		}
		/*
	    transition.relfexive = true;
    ...
    function mousedown()
    {
		...
		//for touch: check if over clearRect
		var point = d3.mouse(this);
		if(point[0] <= width && point[0] >= width - clearRect_width
			&& point[1] <= clearRect_height && point[0] >= 0)
		{
			overClear = true;
		}
		else
		...
		//for touch: check if over trash label
		var trash_x = Number(trashLabel.attr('x')),
		trash_y = Number(trashLabel.attr('y')),
		trash_width = Number(trashLabel.attr('width')),
		trash_height = Number(trashLabel.attr('height'));

		if(point[0] >= trash_x && point[0] <= trash_x + trash_width
			&& point[1] <= trash_y + trash_height && point[1] >= trash_y)
		{
			overTrash = true;
		}
		else
    ...
    function mouseup()
    {
		...
		var point = d3.mouse(this);
		//for touch: ckeck if over trashlabel, for removing nodes
	    var trash_x = Number(trashLabel.attr('x')),
		trash_y = Number(trashLabel.attr('y')),
		trash_width = Number(trashLabel.attr('width')),
		trash_height = Number(trashLabel.attr('height'));
		...
		if(mousedown_node)
		{
		    ...
		    {
		    	if(point[0] >= nodes[n].x - config.node.radius &&
		    		point[0] <= nodes[n].x + config.node.radius &&
		    		point[1] >= nodes[n].y - config.node.radius &&
		    		point[1] <= nodes[n].y + config.node.radius)
		    	{
		    		if(nodes[n].id == 0)
		    		{
		    			circle.filter(":nth-child(1)").select(".node").dispatch("mouseup");
		    		}else{
    ...
    /**
    * Sets angles for all nodes
    * to use in dynamic drawing of selfLoops
    */
    function setselfLoopangles(l)
    {
    	var node_angle = 3/2 * Math.PI;

    	for (var n = 0; n < nodes.length; n++)  
    	{
    		for (var i = 0; i < l.length; i++)
    		{
    			if(l[i].source.id != l[i].target.id) 
    			{
    				if(l[i].source.id == nodes[n].id)
    				{
    					var x1 = l[i].source.x,
					    y1 = l[i].source.y,
					    x2 = l[i].target.x,
						...
						if((angle <= node_angle + 1/4 * Math.PI && angle >= node_angle - 1/4 * Math.PI) ||
							angle <= node_angle - 2 * Math.PI + 1/4 * Math.PI && angle >= node_angle - 2 * Math.PI - 1/4 * Math.PI)
						{
							node_angle = angle + 1/4 * Math.PI;  
						}

    				}
    				else if(l[i].target.id == nodes[n].id)
    				{
    					var x1 = l[i].target.x,
					    y1 = l[i].target.y,
					    x2 = l[i].source.x,
			...
			nodes[n].selfLoopangle = node_angle;
			node_angle = 3/2 * Math.PI;
    ...
    {
    	//nodes in radial force become draggable
    	if(layoutForce == "radial" && d.useRadial)
    	{
    		d.useRadial = false;
    	}
    ...
    function dragend(d, i)
    {
    	...
    	//needed for deleting nodes with touch
    	var trash_x = Number(trashLabel.attr('x')),
		trash_y = Number(trashLabel.attr('y')),
		trash_width = Number(trashLabel.attr('width')),
		trash_height = Number(trashLabel.attr('height'));

		if(d.x >= trash_x && d.x <= trash_x + trash_width
			&& d.y <= trash_y + trash_height && d.y >= trash_y)
		{
			overTrash = true;
		}
		else
    ...
    function addNode(x, y) {
		...
		var node = {
			...
			priority: 0,
			useRadial : true,
			//used for dynamic relocating of reflexive links
			selfLoopangle : 3 * Math.PI / 2
		};
		node.x = x;
		node.y = y;
		nodes.push(node);
	...
	if(!started){
		//Testing menu for choosing forces
		//createForceTestingMenu();
	    ...
	    //optional tutorial message for controls
	    //controlTutorial();
	}
