...
namespace AutomataPDL.Automata
{
    public static class AutomataUtilities
    {
        ...
        */
        public static DFA<char, string> ParseDFAFromXML(XElement automaton_wrapped) //tahiti
        {
        ...
        public static NFA<char, string> ParseNFAFromXML(XElement automaton_wrapped) //tahiti
        {
            var Q = new HashSet<State<string>>();
            var Sigma = new HashSet<char>();
        ...
        */
        ...
        private static bool charToInt(char symbol, int blank, out int intSymbol)
        {
            intSymbol = 0;
            bool rv = false;
            if (symbol == '0')
        ...
        public static bool GetFunctionModeOfTuringMachine(XElement automaton_wrapped)
        {
            XElement automaton = XElement.Parse(RemoveAllNamespaces(automaton_wrapped.ToString()));
            XElement tmData = automaton.Element("tmData");
            bool isFunction = true;
            foreach (XElement child in tmData.Elements())
            {
                if (child.Name == "tmFunction")
                {
                    isFunction = Convert.ToString(child.Value).Equals("true");
                }
            }
            return isFunction;
        }

        public static int GetNumberOfTapesOfTuringMachine(XElement automaton_wrapped)
        {
            XElement automaton = XElement.Parse(RemoveAllNamespaces(automaton_wrapped.ToString()));
            XElement tmData = automaton.Element("tmData");
            int number = 1;
            foreach (XElement child in tmData.Elements())
            {
                if (child.Name == "numberOfTapes")
                {
                    number = Int32.Parse(child.Value);
                }
            }
            return number;
        }

        //public static TM<char, string> GetCorrectSolutionTM<S>(XElement tmCorrectDesc, string boolValue)
        //{
        //    return;
        //}
