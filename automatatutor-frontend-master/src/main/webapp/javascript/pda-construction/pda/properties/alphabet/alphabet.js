...
const Alphabet = class extends Property {
    ...
    exportToXml() {
        return xmlExporter.buildXmlString().addElement(XML_ALPHABET_ID,
            xmlExporter.buildXmlStringFromArray(this.alphabetWithoutEpsilon, XML_SYMBOL_ID)).build().build();
    }
    ...
    _createView() {
        this._view = new AlphabetView(this._htmlElement, alphabet, this._alphabetViewListener(),
            this.alphabetWithoutEpsilon, this._immutable, () => this._automaton.enableEditing);
    }
    ...
    set alphabet(value) {
        ...
        this._view.changeProperty(this.alphabetWithoutEpsilon);
    }
