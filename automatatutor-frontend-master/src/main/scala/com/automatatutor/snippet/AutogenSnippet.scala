...
class Autogensnippet {
  ...
  def autogenproblemlist(xhtml: NodeSeq): NodeSeq = {
	...
	return TableHelper.renderTableWithHeader(
			...
			("", (problem: Problem) => {(new ProblemRenderer(problem)).renderDeleteLink("/autogen/index")})) ++ deleteAllLink
  }
  ...
  def movetocourseform(xhtml: NodeSeq): NodeSeq = {
	...
	if (supervisedCourses.isEmpty) return Text("You do not supervise any course.")
	
	val courseOptions = Array(supervisedCourses.map(course => (course, course.getName)))
    ...
    return TableHelper.renderTableWithHeader(
            ...
            ("", (course: Course) => SHtml.link(
			  "/autogen/index", 
			  () => {CurrentPracticeProblem.is.setCourse(course).save}, 
			  <button type='button'>Send</button>))) ++ cancelButton
  }
}